#' IPHRA Anthropometric Data Quality Class
#'
#' The `AnthropometricDataQuality` R6 class extends `DataQuality` to provide
#' Anthropometric-specific quality checks and analytics.
#'
#' @description
#' This class provides specialized quality checks for anthropometric data including:
#' * Height/length measurement plausibility
#' * Weight measurement plausibility
#' * MUAC measurement validation
#' * Z-score validation
#' * Age-based plausibility checks
#'
#' @seealso [DataQuality]
#' @export
AnthropometricDataQuality <- R6::R6Class(
  classname = "AnthropometricDataQuality",
  inherit = DataQuality,

  public = list(

    #' @description
    #' Initialize a new AnthropometricDataQuality object
    #'
    #' @param data A data frame (standardized or clean anthropometric data)
    ...
    initialize = function(data = NULL,
                          ...
                          dataset_name = "AnthropometricDataQuality",
      ...
      iphra_message(
        iphra_txt(glue::glue("{dataset_name} initialized as AnthropometricDataQuality object."))
      )
    },

    #' @description
    #' Get the default Anthropometric quality schema from template file
    #'
    #' @return A list of anthropometric-specific quality checks (the schema itself)
    default_quality_schema = function() {
      ...
      df <- tryCatch(
        ...
        error = function(e) {
          iphra_warning(
            origin  = "AnthropometricDataQuality$default_quality_schema",
            message = iphra_txt(glue::glue("Failed to read quality_schema_data_quality_anthropometric_template.xlsx: {e$message}"))
          )
          return(NULL)
        }
      )
      ...
      df <- tryCatch(
        ...
        error = function(e) {
          iphra_warning(
            ...
            message = iphra_txt(glue::glue("Failed to read outputs_quality_schema_data_quality_anthropometric_template.xlsx: {e$message}"))
          )
          return(NULL)
        }
      )
