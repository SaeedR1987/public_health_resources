...
Document <- R6::R6Class(
  ...
  public = list(
      ...
      phr_try(
        {
          ...
          doc <- private$..apply_protocol_schema_sections(
            doc = doc,
    ...
    #' @return Invisibly returns \code{self}.
    generate_ppt = function(
      output_file = "protocol_report.pptx",
      open = FALSE
    ) {
      phr_try(
        {
          ...
          ppt <- private$..apply_protocol_schema_sections(
            doc = ppt,
  ...
  private = list(
    ...
    ..create_base_doc = function(reference_doc_filename = NULL) {
      if (
        !is.null(reference_doc_filename) && file.exists(reference_doc_filename)
      ) {
    ...
    ..create_base_ppt = function(reference_ppt_filename = NULL) {
      if (
        !is.null(reference_ppt_filename) && file.exists(reference_ppt_filename)
      ) {
    ...
    #' @description
    #' Generate a document from schema and apply protocol schema sections.
    #' @param obj Document object (\code{officer::rdocx} or \code{officer::rpptx}).
    #' @param schema Data frame describing document structure and content.
    #' @param schema_kind Document kind (\code{"docx"} or \code{"pptx"}).
    #' @return Updated document object.
    ..generate_from_schema = function(
      obj,
      schema,
      schema_kind = c("docx", "pptx")
    ) {
      schema_kind <- match.arg(schema_kind)
      private$..apply_protocol_schema_sections(
        doc = obj,
        schema = schema,
    ...
    #'
    #' Processes schema rows by handling type (replace, input, calculate,
    #' checkbox_replace, row_delete, table, image) in a consistent sequence.
    #' Validates required columns and evaluates row conditions before applying
    #' each handler.
    #'
    #' @param doc \code{officer::rdocx} or \code{officer::rpptx} document object.
    #' @param schema Data frame with columns: tag_name, handling, condition,
    #'   default_value, function_name.
    #' @param schema_kind Document kind (\code{"docx"} or \code{"pptx"}).
    #' @return Updated document object with schema sections applied.
    ..apply_protocol_schema_sections = function(
      doc,
      schema = self$protocol_schema,
      schema_kind = c("docx", "pptx")
    ) {
      ...
      if (!all(required_cols %in% names(schema))) {
        phr_warning(
          phr_txt(
            "protocol_schema missing required columns: {paste(setdiff(required_cols, names(schema)), collapse=', ')}"
          ),
      ...
      for (handling in handling_order) {
        schema_handling <- if (
          "handling" %in% names(schema) && !is.null(schema$handling)
        ) {
          as.character(schema$handling)
        } else {
          rep("", nrow(schema))
        }
        normalized_handling <- ifelse(
          schema_handling == "conditional_replace",
          "replace",
          schema_handling
        )
        ...
        for (i in idx) {
          ...
          doc <- switch(
            handling,
            replace = private$..handle_replace(
              doc,
              row,
              schema_kind = schema_kind
            ),
            input = private$..handle_input(doc, row, schema_kind = schema_kind),
            calculate = private$..handle_calculate(
              doc,
              row,
              schema_kind = schema_kind
            ),
            checkbox_replace = private$..handle_checkbox_replace(
              doc,
              row,
              schema_kind = schema_kind
            ),
            row_delete = private$..handle_row_delete(
              doc,
              row,
              schema_kind = schema_kind
            ),
    ...
    #' @description
    #' Handle a schema row with \code{handling == "replace"}.
    #'
    #' Replaces a tag with its default value if the row condition is met.
    #'
    #' @param doc \code{officer::rdocx} or \code{officer::rpptx} document object.
    ...
    ..handle_replace = function(doc, row, schema_kind = c("docx", "pptx")) {
      schema_kind <- match.arg(schema_kind)
      if (!isTRUE(private$..should_apply_schema_row(row))) {
        return(doc)
      }
    ...
    #' @description
    #' Handle a schema row with \code{handling == "input"}.
    #'
    #' Replaces a tag with a value from \code{self$metadata} if the row
    #' condition is met.
    #'
    #' @param doc \code{officer::rdocx} or \code{officer::rpptx} document object.
    #' @param row Single-row schema data frame.
    ...
    ..handle_input = function(doc, row, schema_kind = c("docx", "pptx")) {
      ...
      tag <- as.character(row$tag_name[[1L]] %||% "")
      key <- sub("^@", "", tag)
      value <- if (nzchar(key) && key %in% names(self$metadata)) {
        self$metadata[[key]]
      } else {
        ""
      }
    ...
    #' @description
    #' Evaluate the condition for a schema row.
    #'
    #' Resolves condition values from active bindings or metadata. Returns
    #' \code{TRUE} if no condition is specified.
    #'
    #' @param row Single-row schema data frame.
    #' @return Logical value indicating whether row condition is met.
    ..evaluate_condition_row = function(row) {
      if (
        !is.data.frame(row) || nrow(row) == 0L || !"condition" %in% names(row)
      ) {
        return(TRUE)
      }
      ...
      binding_name <- private$..resolve_active_binding_name(sub(
        "^@",
        "",
        condition
      ))
      if (is.null(binding_name)) {
        return(private$..resolve_condition_flag(condition))
      }
      value <- private$..read_active_binding(binding_name)
      if (is.null(value)) {
        return(private$..resolve_condition_flag(condition))
      }
      value
    ...
    #' @description
    #' Handle a schema row with \code{handling == "calculate"}.
    #'
    #' Evaluates an active binding and replaces the tag with its computed
    #' value if the row condition is met. Skips replacement for boolean values
    #' which are treated as control signals only.
    #'
    #' @param doc \code{officer::rdocx} or \code{officer::rpptx} document object.
    #' @param row Single-row schema data frame.
    #' @param schema_kind Document kind (\code{"docx"} or \code{"pptx"}).
    ...
    #' @description
    #' Handle a schema row with \code{handling == "checkbox_replace"}.
    #'
    #' Replaces a tag with a checkbox symbol ("X" or "☐") based on a boolean
    #' value from \code{self$metadata} if the row condition is met.
    #'
    #' @param doc \code{officer::rdocx} or \code{officer::rpptx} document object.
    #' @param row Single-row schema data frame.
    #' @param schema_kind Document kind (\code{"docx"} or \code{"pptx"}).
    #' @return Updated document object.
    ..handle_checkbox_replace = function(
      doc,
      row,
      schema_kind = c("docx", "pptx")
    ) {
      ...
      private$..replace(
        doc,
        tag,
        if (isTRUE(value)) "X" else "\u25a1",
        schema_kind = schema_kind
      )
    },

    #' @description
    #' Handle a schema row with \code{handling == "row_delete"}.
    #'
    #' Removes a tag from the document by replacing it with an empty string
    #' if the row condition is met.
    #'
    #' @param doc \code{officer::rdocx} or \code{officer::rpptx} document object.
    #' @param row Single-row schema data frame.
    #' @param schema_kind Document kind (\code{"docx"} or \code{"pptx"}).
    ...
    ..handle_table = function(doc, row, schema_kind = c("docx", "pptx")) {
      ...
      tbl <- private$..dispatch_schema_function(
        row = row,
        schema_kind = schema_kind
      )
    ...
    ..handle_image = function(doc, row, schema_kind = c("docx", "pptx")) {
      ...
      tag <- as.character(row[["tag_name"]][[1L]] %||% "")
      tag_binding <- private$..resolve_active_binding_name(sub(
        "^@",
        "",
        trimws(tag)
      ))
      if (!is.null(tag_binding)) {
        img <- private$..read_active_binding(tag_binding)
        if (!is.null(img)) {
          return(private$..insert_image_at_tag(
            doc = doc,
            tag = tag,
            image_value = img,
            schema_kind = schema_kind
      ...
      out <- private$..dispatch_schema_function(
        row = row,
        schema_kind = schema_kind
      )
      if (is.null(out)) {
    ...
    ..load_protocol_schema = function() {
      ...
      empty_schema <- as.data.frame(
        setNames(
          replicate(length(required_cols), character(0), simplify = FALSE),
          required_cols
        ),
      ...
      if (identical(schema_kind, "pptx")) {
        doc <- tryCatch(
          ...
          error = function(e) {
            phr_warning(
              phr_txt(
                "PPTX placeholder '{old}' could not be replaced: {conditionMessage(e)}"
              ),
    ...
    ..read_active_binding = function(binding_name) {
      if (
        !is.character(binding_name) ||
          length(binding_name) != 1L ||
          !nzchar(binding_name)
      ) {
        return(NULL)
      }
    ...
    ..insert_image_at_tag = function(
      ...
      if (identical(schema_kind, "pptx")) {
        if (
          is.character(image_value) &&
            length(image_value) == 1L &&
            file.exists(image_value)
        ) {
          return(tryCatch(
            {
              officer::ph_with(
    ...
    ..dispatch_schema_function = function(
      row,
      schema_kind = c("docx", "pptx")
    ) {
      schema_kind <- match.arg(schema_kind)
      ...
      args <- private$..schema_dispatch_args(
        fn_name = fn_name,
        row = row,
        schema_kind = schema_kind
      )
    ...
    ..schema_dispatch_args = function(
      ...
      if (
        is.null(sample_table) &&
          !is.null(self$sample_object) &&
          inherits(self$sample_object, "Sample")
      ) {
    ...
    ..tag_is_missing_from_schema = function(tag) {
      startsWith(as.character(tag %||% ""), "@") &&
        is.null(private$..schema_row(tag))
    },
    ...
    ..replace_tag_in_cell = function(doc, tag, items) {
      ...
      for (item in items) {
        node <- private$..make_w_para(
          ...
          space_before_pt = if (is.null(item$space_before_pt)) {
            0L
          } else {
            item$space_before_pt
          },
          space_after_pt = if (is.null(item$space_after_pt)) {
            0L
          } else {
            item$space_after_pt
          },
    ...
    ..remove_remaining_tags = function(doc) {
      ...
      for (para in paras) {
        ...
        if (!grepl(tag_pattern, combined, perl = TRUE)) {
          next
        }
    ...
    ..resolve_condition_flag = function(condition) {
      ...
      if (norm %in% c("true", "false")) {
        return(identical(norm, "true"))
      }

      key <- sub("^@", "", condition)
      if (
        is.list(self$conditional_metadata) &&
          key %in% names(self$conditional_metadata)
      ) {
    ...
    .._replace_across_runs = function(doc, tag, new_val) {
      ...
      for (para in paras) {
        repeat {
          ...
          combined <- paste(texts, collapse = "")
          nc <- nchar(combined)
          if (nc == 0L || !grepl(tag, combined, fixed = TRUE)) {
            break
          }
          ...
          for (m in as.integer(matches)) {
            ...
            right_char <- if (end < nc) {
              substr(combined, end + 1L, end + 1L)
            } else {
              ""
            }
            left_ok <- !nzchar(left_char) ||
              !grepl("[A-Za-z0-9_.\\-]", left_char, perl = TRUE)
            right_ok <- !nzchar(right_char) ||
              !grepl("[A-Za-z0-9_.\\-]", right_char, perl = TRUE)
            if (left_ok && right_ok) {
              match_start <- m
              break
            }
          ...
          if (tag_start > 1L) {
            pre_chars <- strsplit(
              substr(combined, 1L, tag_start - 1L),
              "",
              fixed = TRUE
            )[[1L]]
            pre_runs <- node_idx[seq_len(tag_start - 1L)]
            for (j in seq_along(pre_chars)) {
              new_texts[pre_runs[j]] <- paste0(
                new_texts[pre_runs[j]],
                pre_chars[j]
              )
            }
          }
          ...
          if (tag_start > length(node_idx)) {
            phr_warning(
              phr_txt(
                "._replace_across_runs: tag_start ({tag_start}) exceeds node_idx length ({length(node_idx)}) for tag '{tag}'; skipping paragraph."
              ),
          ...
          if (tag_end < nc) {
            suf_chars <- strsplit(
              substr(combined, tag_end + 1L, nc),
              "",
              fixed = TRUE
            )[[1L]]
            suf_runs <- node_idx[seq(tag_end + 1L, nc)]
            for (j in seq_along(suf_chars)) {
              new_texts[[suf_runs[j]]] <- paste0(
                new_texts[[suf_runs[j]]],
                suf_chars[j]
              )
            }
          }
