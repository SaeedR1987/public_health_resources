...
DemographicsAnalysis <- R6::R6Class(
  ...
  public = list(
    ...
    default_outputs_schema = function() {
      file <- system.file(
        "resources",
        "outputs_analysis_schema_quant_data_analysis_demographics_template.xlsx",
        package = "iphRa"
      )

      if (!file.exists(file) || file == "") {
        file <- file.path("resources", "outputs_analysis_schema_quant_data_analysis_demographics_template.xlsx")
        if (!file.exists(file)) {
          return(list())
        }
      }
      ...
      df <- iphra_try(
        ...
        hint = "Check that outputs_analysis_schema_quant_data_analysis_demographics_template.xlsx is a valid Excel file."
      )
