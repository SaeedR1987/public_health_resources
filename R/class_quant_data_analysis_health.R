#' HealthAnalysis
#'
#' @description A class for health-specific quantitative data analysis
#'
#' @noRd
HealthAnalysis <- R6::R6Class(
  classname = "HealthAnalysis",
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new HealthAnalysis object
    #'
    ...
    #' @param linked_ind_health_data Optional linked HealthIndividualData dataframe
    ...
    #' @return A new HealthAnalysis object
    initialize = function(data = NULL,
                          dap = NULL,
                          parent_data_object = NULL,
                          dataset_name = "HealthAnalysis",
    ...
    #' @description
    #' Load the default analysis schema from the Health-specific template file
    #'
    ...
    #' @description
    #' Load the default outputs schema from template file for Health analysis
    #'
    #' @return A list containing the outputs schema
    default_outputs_schema = function() {
      file <- system.file(
        "resources",
        "outputs_analysis_schema_quant_data_analysis_health_template.xlsx",
        package = "iphRa"
      )

      if (!file.exists(file) || file == "") {
        file <- file.path("resources", "outputs_analysis_schema_quant_data_analysis_health_template.xlsx")
        if (!file.exists(file)) {
          return(list())
        }
      }
      ...
      df <- phr_try(
        ...
        hint = "Check that outputs_analysis_schema_quant_data_analysis_health_template.xlsx is a valid Excel file."
      )
