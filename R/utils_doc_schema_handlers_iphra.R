...
.tool_indicator_codes <- function(tools, tool_names = NULL, prefer_revised = TRUE) {
  if (is.null(tools) || length(tools) == 0L) return(character(0))
...
table_primary_data_sources <- function(master_schema, tool_indicator_codes) {
  ...
  if (!is.list(tool_indicator_codes) || length(tool_indicator_codes) == 0L) return(NULL)

  tool_names <- names(tool_indicator_codes)
  all_codes <- unique(unlist(tool_indicator_codes, use.names = FALSE))
  if (length(all_codes) == 0L) return(NULL)

  obj_sub <- unique(master_schema[as.character(master_schema$indicator_code) %in% all_codes,
                                  c("objective_code", "text_objective"), drop = FALSE])
  obj_sub <- obj_sub[!is.na(obj_sub$objective_code), , drop = FALSE]
  if (nrow(obj_sub) == 0L) return(NULL)
  ...
  for (j in seq_along(tool_names)) {
    ...
    obj_covered <- unique(as.character(master_schema$objective_code[as.character(master_schema$indicator_code) %in% tc]))
...
table_secondary_data_sources <- function(master_schema, secondary_data) {
  if (!is.null(secondary_data) && length(secondary_data) > 0) {
    obj_labels <- setNames(vapply(names(secondary_data), function(code) {
      if (is.data.frame(master_schema) && all(c("objective_code", "text_objective") %in% names(master_schema))) {
        idx <- which(as.character(master_schema$objective_code) == code)
        if (length(idx) > 0) return(as.character(master_schema$text_objective[idx[1]]))
      }
      code
    }, character(1)), names(secondary_data))
    sdr_df <- data.frame(Objective = unname(obj_labels[names(secondary_data)]),
  ...
  if (!is.null(secondary_data) && length(secondary_data) > 0) ft <- flextable::merge_v(ft, j = "Objective")
...
.table_sample_size_builder <- function(sample_table, param_rows, total_labels) {
  if (is.null(sample_table) || !is.data.frame(sample_table) || nrow(sample_table) == 0L) return(NULL)

  strata_names <- if ("stratum_name" %in% names(sample_table)) as.character(sample_table$stratum_name) else as.character(sample_table$stratum_id)
  n_strata <- length(strata_names)
  col_names <- c("Parameter", strata_names, "Justification")
  rows_list <- lapply(param_rows, function(pr) {
    vals <- vapply(seq_len(n_strata), function(j) tryCatch(pr$col_fn(sample_table[j, , drop = FALSE]), error = function(e) ""), character(1L))
  ...
  flextable::hline_bottom(ft, border = fp_outer, part = "header")
}
...
table_sample_size_general <- function(sample_table) {
  ...
  .table_sample_size_builder(sample_table, params,
