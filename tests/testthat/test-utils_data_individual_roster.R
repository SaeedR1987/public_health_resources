...
test_that("add_age_months_cat() — valid dataset creates age month categories", {
  ...
  # Check age_6_29m and age_30_59m columns exist
  expect_true("age_6_29m" %in% names(out))
  expect_true("age_30_59m" %in% names(out))
})

test_that("add_age_months_cat() — age_6_29m and age_30_59m values are correct", {
  ...
  expect_true(is.na(out$age_6_29m[1]))
  expect_true(is.na(out$age_6_29m[2]))
  expect_true(is.na(out$age_30_59m[1]))
  expect_true(is.na(out$age_30_59m[2]))

  # 6-29 months → age_6_29m = 1, age_30_59m = 0
  expect_equal(out$age_6_29m[3], 1)
  expect_equal(out$age_6_29m[4], 1)
  expect_equal(out$age_30_59m[3], 0)
  expect_equal(out$age_30_59m[4], 0)

  # 30-59 months → age_6_29m = 0, age_30_59m = 1
  expect_equal(out$age_6_29m[5], 0)
  expect_equal(out$age_6_29m[6], 0)
  expect_equal(out$age_30_59m[5], 1)
  expect_equal(out$age_30_59m[6], 1)

  # 60+ months → NA for both columns
  expect_true(is.na(out$age_6_29m[7]))
  expect_true(is.na(out$age_30_59m[7]))
})
